...
class graphLedger(val module: DefModule) {
  ...
  */
  ...
  def getInstances: Set[DefInstance] = {
    print(s"defInstances size: ${defInstances.size}\n")
    for (inst <- defInstances) {
      if (inst.getClass.getSimpleName != "DefInstance")
        throw new Exception(s"${inst.serialize} is not DefInstance class, but ${inst.getClass.getSimpleName}")
      else
        print(s"Is a DefInstance: ${inst.asInstanceOf[DefInstance].name}\n")
    }
