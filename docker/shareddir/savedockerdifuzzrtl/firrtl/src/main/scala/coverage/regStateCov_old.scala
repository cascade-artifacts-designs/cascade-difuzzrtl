...
class Ledger {
  ...
  var moduleInst = mutable.Map[String, List[DefInstance]]()
  ...
  def foundInst(defInst: DefInstance): Unit = moduleName match {
    case None => sys.error("Module name not defined in Ledger!")
    case Some(name) =>
      moduleInst(name) = moduleInst.getOrElse(name, List[DefInstance]()) :+ defInst
    case _ => ()
  }
...
class StateCoverage extends Transform {
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    for (key <- ledger.modules) {
      val listInst = ledger.moduleInst.getOrElse(key, List[DefInstance]())
  ...
  def instrCov(ledger: Ledger, max_state_width: Int = 20)(m: DefModule): DefModule = {
    ...
    m match {
      case mod: Module  =>
        ...
        val instances: Seq[DefInstance] = ledger.moduleInst.getOrElse(mod.name, List[DefInstance]()).toSeq
  ...
  def portConnect(instances: Seq[DefInstance], port: Port, covSum: Statement, max_state_width: Int): Seq[Statement] = {
  ...
  def makeSum(instances: Seq[DefInstance], port: Port, sum: Statement, max_state_width: Int): Seq[Statement] = {
  ...
  def instrReset(ledger: Ledger)(m: DefModule): DefModule = {
    ...
    m match {
      case mod: Module =>
        ...
        val stmts = block.stmts
        val instances: Seq[DefInstance] = ledger.moduleInst.getOrElse(newModule.name, List[DefInstance]()).toSeq
